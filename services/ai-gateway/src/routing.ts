import type { ProviderName, AgentCategory, ProviderRoute } from "@aiverse/shared";
...
export function rotateApiKey(provider: ProviderName): void {
  const env = getEnv();
  const keys: string[] = [];
  const key1 = apiKeyAccessor[provider](env);
  if (key1) keys.push(key1);
  ...
  if (keys.length > 1) {
    ...
    console.log(`Rotated key for provider ${provider} to index ${next}`);
  }
}
