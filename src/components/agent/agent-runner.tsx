    ...
    if (part.startsWith("```") && part.endsWith("```")) {
      ...
      return (
        <div key={i} className="relative group my-2">
          <pre className="bg-zinc-950 rounded-lg p-4 overflow-x-auto text-sm font-mono leading-relaxed border border-zinc-700">
            {lang && <div className="text-xs text-zinc-500 mb-2">{lang}</div>}
            <code>{code}</code>
          </pre>
          <button
            onClick={() => navigator.clipboard.writeText(code)}
            className="absolute top-2 right-2 text-xs px-2 py-1 rounded bg-zinc-700 text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-zinc-600"
          >
            Copy
          </button>
        </div>
    ...
    <div className={`flex ${role === "user" ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[80%] rounded-lg px-4 py-2 text-sm ${
          role === "user" ? "bg-purple-600 text-white" : "bg-zinc-800 text-zinc-200"
        }`}
      >
  ...
  return (
    <div className="px-4 py-2 border-b border-zinc-800">
      <div className="flex items-center gap-2 text-xs text-zinc-500">
        ...
        <span className="text-zinc-600">Step {steps.length}</span>
      </div>
    </div>
  );
}
...
export function AgentRunner({ agentId, slug, agentName, category, systemPrompt, isTestMode, isCreator }: AgentRunnerProps) {
  ...
  return (
    <div className="flex flex-col h-[600px]">
      ...
      <div className="flex-1 overflow-y-auto space-y-4 p-4">
        {messages.length === 0 && (
          <div className="text-center py-12">
            ...
            {isDataAgent && (
              <p className="text-xs text-zinc-600 mt-2">Attach a CSV, XLSX, PDF, or TXT file for analysis</p>
            )}
            {isCreator && (
      ...
      <form onSubmit={handleSubmit} className="border-t border-zinc-800 p-4 flex flex-col gap-2">
        {isDataAgent && (
